...
const cardNameToCardNameKey = (name) =>
    name.replace(cardNameReplacementRegex, '').replace(spaceRegex, '-').toLowerCase();
...
const main = async (dbPool) => {
    ...
    for (let i = 0; i < cards.length; i += 1) {
        ...
        let ffgData = DTCards.cards.find(
            (data) =>
                data.id === cardNameToCardNameKey(card) ||
                stripSpecialCharacters(data.name) === stripSpecialCharacters(card)
        );
        ffgData =
            ffgData ||
            MMCards.cards.find(
        ...
        ffgData =
            ffgData ||
            WCCards.cards.find(
                (data) => data.id === cardNameToCardNameKey(card) || data.name === card
            );
        ffgData =
            ffgData ||
            AoACards.cards.find(
