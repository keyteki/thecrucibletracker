...
const getNextGameIDs = async (dbPool) => {
  const query = 'SELECT id FROM games WHERE scanned_for_card_plays IS FALSE AND card_plays_hit_error IS FALSE LIMIT 10';
...
const consume = (dbPool) => {
  setInterval(async () => {
    ...
    for (const gameID of ids) {
      ...
      events.forEach((e) => {
        try {
          if (Array.isArray(e.message) && / plays /.test(e.message[1])) {
            ...
            cardPlays[e.message[0].name][card] = cardPlays[e.message[0].name][card] || { played: 0, };
            cardPlays[e.message[0].name][card].played += 1;
          }

          if (
            Array.isArray(e.message) && / discards /.test(e.message[1])
          ...
          && !e.message[3]
            ...
            cardDiscards[e.message[0].name][card] = cardDiscards[e.message[0].name][card] || { count: 0, };
      ...
      Object.keys(cardPlays)
        .forEach((player) => {
          Object.keys(cardPlays[player])
            .forEach((card) => {
      ...
      await clientForCardPlays.query(`UPDATE games SET scanned_for_card_plays = TRUE WHERE id = ${gameID}`);
