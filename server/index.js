...
const loadSetupData = async () => {
  try {
    const query = 'SELECT date, winner, loser, turns, crucible_game_id FROM games ORDER BY date DESC LIMIT 5';
...
games.on('new-game', ({
  ...
  const dup = recentGames.find((game) => game.winner === winner && game.loser === loser && game.turns === turns);

  setTimeout(() => {
    if (!dup) {
      recentGames.pop();
      recentGames.unshift(cleanGame({
        winner,
        loser,
        date,
        turns,
