const getCardTypePlayedForPlayer = (player, cardType, events) => events.filter((e) => Array.isArray(e.message)
      && e.message[0].name === player
      && / plays /.test(e.message[1])
      && e.message[2].argType === 'card'
      && e.message[2].type === cardType).length;
...
const getCardsDiscardedForPlayer = (player, events) => {
  const discardCardEvents = events.filter((event) => Array.isArray(event.message)
