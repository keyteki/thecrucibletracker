...
const getChainsGained = (player, events) => {
  const chainReducedEvents = events.filter((event) => Array.isArray(event.message) && event.message[0].name === player
...
const getStartingChains = (player, events) => {
  const endOfTurn1Index = findLastIndex(events, (event) => event.message && event.message.alert && Array.isArray(event.message.alert.message)
    && event.message.alert.message[0] === 'End of turn 1');

  const firstChainReduction = events.slice(0, endOfTurn1Index).find((event) => Array.isArray(event.message) && event.message[0].name === player
    && /s chains are reduced by 1 to /.test(event.message[1]));
