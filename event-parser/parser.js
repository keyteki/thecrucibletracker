...
const { getKeysForPlayer } = require('./keys');

const { getCardTypePlayedForPlayer, getCardsDiscardedForPlayer } = require('./cards');

const { getTurnsPerHouseForPlayer } = require('./house');

const { getChainsGained, getStartingChains } = require('./chains');
...
const checkMulligan = (player, events) => {
    ...
    while (firstTurnIndex < events.length) {
        ...
        if (
            ...
            /End of turn/.test(event.message.alert.message[0])
        ) {
            break;
        }
    ...
    const shuffles = pregameEvents.filter(
        (event) =>
            event.message &&
            event.message[0] &&
            event.message[0].name &&
            event.message[0].name === player &&
            event.message[1] &&
            / is shuffling their deck/.test(event.message[1])
    ).length;
...
const getTurns = (events) => {
    ...
    events.forEach((e) => {
        if (
            e.message &&
            e.message.alert &&
            e.message.alert.type === 'startofround' &&
            e.message.alert.message[0] &&
            /Turn/.test(e.message.alert.message[0])
        ) {
            turns += 0.5;
        }
    });
...
const createGameSummary = (events) => {
    ...
    const playerADeckEvent = events.find(
        (e) =>
            e.message &&
            (e.message[1] === ' brings ' || e.message[1].includes('playing as')) &&
            e.message[2].link &&
            e.message[2].link.indexOf('undefined') === -1
    );
    const playerADeckID = playerADeckEvent.message[2].link.replace(
        'https://www.keyforgegame.com/deck-details/',
        ''
    );
    const playerADeckName = playerADeckEvent.message[2].label;

    const playerBDeckEvent = events.find(
        (e) =>
            e.message &&
            (e.message[1] === ' brings ' || e.message[1].includes('playing as')) &&
            e.message[2].link &&
    ...
    const playerBDeckID = playerBDeckEvent.message[2].link.replace(
        'https://www.keyforgegame.com/deck-details/',
        ''
    );
