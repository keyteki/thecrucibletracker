...
const parse = (events) => {
    ...
    const firstPlayer = events.find(
        (event) => event.message && event.message.alert && event.message.alert.type === 'endofround'
    ...
    for (let i = 0; i < events.length; i++) {
        ...
        if (Array.isArray(event.message) && /: \d+ amber .*: \d+ amber/.test(event.message[0])) {
            ...
            const activePlayer =
                turnAt % 2 === 0 ? (firstPlayer === playerA ? playerB : playerA) : firstPlayer;
