...
const fetchForUser = (user, deckID) => {
    ...
    return Promise.all([
        ...
        if (deckID) {
            games = games.filter(
                (g) =>
                    (g.winner === user && g.winner_deck_id === deckID) ||
                    (g.loser === user && g.loser_deck_id === deckID)
            );
        }
...
class GamesScreen extends Component {
    ...
    componentDidMount(prevProps, prevState, snapshot) {
        ...
        fetchForUser(this.props.user, this.props.deckID).then(({ games, gameSummaries }) => {
            this.setState({
                games,
                gameSummaries
            });
    ...
    render() {
        ...
        const { games, gameSummaries, awards } = this.state;
