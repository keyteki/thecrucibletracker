...
class Table extends Component {
    render() {
        let { games, gameSummaries, awards } = this.props;
        const { user, deckID, isMobile } = this.props;

        let deckName = '';
        if (games[0]) {
            deckName =
                games[0].winner_deck_id === deckID
                    ? games[0].winner_deck_name
                    : games[0].loser_deck_name;
        }
        ...
        gameSummaries = gameSummaries.filter((summary) =>
            games.find((game) => game.id === summary.game_id)
        );
        ...
        gameSummaries.forEach((summary) => {
            ...
            cardsPlayedDiffSum +=
                summary.winner === user
                    ? winnerCardsPlayed - loserCardsPlayed
                    : loserCardsPlayed - winnerCardsPlayed;
        });
                    ...
                    <div>
                        <div className='page-title' style={{ marginBottom: '1rem' }}>
                            <div>
                                ...
                                <a
                                    href={`https://keyforgegame.com/deck-details/${deckID}`}
                                    style={{
                                        textDecoration: 'none'
                                    }}
                                >
                        ...
                        >
                            ...
                            {winsAgaintSets[expansions.cota].length +
                                lossesAgaintSets[expansions.cota].length >
                                0 &&
                                Stat({
                                    title: 'VS CotA',
                                    value: `${(
                            ...
                            {winsAgaintSets[expansions.aoa].length +
                                lossesAgaintSets[expansions.aoa].length >
                                0 &&
                                Stat({
                                    title: 'VS AoA',
                                    value: `${(
                            ...
                            {winsAgaintSets[expansions.wc].length +
                                lossesAgaintSets[expansions.wc].length >
                                0 &&
                                Stat({
                                    title: 'VS WC',
                                    value: `${(
                            ...
                            {winsAgaintSets[expansions.mm].length +
                                lossesAgaintSets[expansions.mm].length >
                                0 &&
                                Stat({
                                    title: 'VS MM',
                                    value: `${(
                            ...
                            {user &&
                                !Number.isNaN(averageCardsPlayedDiff) &&
                                Stat({
                                    ...
                                    value: (
                                        ...
                                        >
                                            {averageCardsPlayedDiff >= 0
                                                ? `+${averageCardsPlayedDiff.toFixed(1)} cards`
                                                : `${averageCardsPlayedDiff.toFixed(1)} cards`}
                                        </span>
                                    )
                                })}
                            {user &&
                                !Number.isNaN(averageCardsPerTurn) &&
                                Stat({
                            ...
                            {user &&
                                !Number.isNaN(gameLengthAverage) &&
                                Stat({
                                    title: 'Avg Length',
                                    value: (
                                        <span
