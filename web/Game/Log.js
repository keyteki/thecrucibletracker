...
class Log extends Component {
  ...
  render() {
    ...
    let firstPlayer;
    if (this.props.log) {
      const firstPlayerAction = this.props.log.find((log) => log.includes(' plays ')) || this.props.log.find((log) => log.includes(' discards '));
      ...
      if (firstPlayerAction) {
        ...
        firstTurnBorderColor = firstPlayer !== game.winner ? 'rgba(252, 127, 121)' : 'rgb(133, 221, 133)';
        const borderColor = firstPlayer === game.winner ? 'rgb(133, 221, 133)' : 'rgba(252, 127, 121)';
        ...
        firstTurnHeader = (
          ...
          >
            <div className="line-content" style={{ display: 'flex', alignItems: 'center', }}>
              {firstPlayer}
            </div>
      ...
      <div>
        ...
        {this.props.log.map((line, i) => {
          ...
          if (/ forges the/.test(line)) {
            const firstPlayerJoinedLine = this.props.log.find((l) => l.indexOf('is playing as the Archon') !== -1);
            ...
            return (
              <div
                key={`line-${i}`}
                style={{
                  padding: '0px', marginBottom: '5px', display: 'inline-block', backgroundColor: '#fffca8',
                }}
          ...
          if (/^Turn \d+/.test(line)) {
            ...
            const playerName = turnInspected % 2 === 1 ? firstPlayer : (game.winner === firstPlayer ? game.loser : game.winner);
            const borderColor = playerName === game.winner ? 'rgb(133, 221, 133)' : 'rgba(252, 127, 121)';
            ...
            const hash = `${firstPlayer === playerName ? 'a' : 'b'}${Math.ceil(turn / 2)}`;
            return (
              <div key={`line-${i}`}>
                <div
                  className="line"
          ...
          if (/: \d+ amber .*: \d+ amber/.test(line)) {
            ...
            return (
              <div key={`line-${i}`} style={{ marginTop: '10px', marginBottom: '10px', }} className="regular-line">
