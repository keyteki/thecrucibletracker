...
class Summary extends Component {
    ...
    render() {
        ...
        if (showTimeGraph) {
            data = {
                labels: _.sortBy(
                    _.keys(_.values(times)[0].turnTimes).map((n) => Number.parseInt(n, 10))
                ),
                series: [
                    _.values(_.values(times)[0].turnTimes),
                    _.keys(times).length > 1
                        ? _.values(_.values(times)[0].turnTimes).map((n) => 0)
                        : [],
                    _.keys(times).length > 1 ? _.values(_.values(times)[1].turnTimes) : []
                ]
            };
                            ...
                            {result === 'winner' ? (
                                ...
                                >
                                    <div className='deck-name'>
                                        {game[result] === 'anonymous' ? (
                                            <span className='deck-name'>anonymous</span>
                                        ) : (
                                            <a
                                                className='deck-name'
                                                href={`/users/${game[result]}`}
                                            >
                                                {game[result]}
                                            </a>
                                        )}
                                        <span style={{ margin: '0 10px 0 5px', fontSize: '20px' }}>
                                            {' '}
                                            using{' '}
                                        </span>{' '}
                                        <a
                                            className='deck-name'
                                            href={`https://keyforgegame.com/deck-details/${
                                                game[`${result}_deck_id`]
                                            }`}
                                        >
                                            {game[`${result}_deck_name`]}
                                        </a>
                                    </div>
                                ...
                                <div className='deck-name'>
                                    {game[result] === 'anonymous' ? (
                                        ...
                                        <a className='deck-name' href={`/users/${game[result]}`}>
                                            {game[result]}
                                        </a>
                                    )}
                                    <span style={{ margin: '0 10px 0 5px', fontSize: '20px' }}>
