      ...
      >
        {creatures.map((card, i) => (
          <Card
            ...
            maxUpgradesOfNeighbors={(card.upgrades || []).length === maxUpgrades ? 0 : maxUpgrades}
          />
        ))}
      </div>
    );
      ...
      >
        ...
        >
          <div style={{ fontSize: '13px', display: 'flex', alignItems: 'center' }}>
            {playerName}
            { Number.isFinite(amber) && <img src={getS3Image('aember')} style={{ width: '25px', }} />}
            { Number.isFinite(keys) && keys > 0 && <img src={getS3Image('key')} style={{ width: '25px', }} /> }
            { Number.isFinite(keys) && keys > 1 && <img src={getS3Image('key')} style={{ width: '25px', }} /> }
            { Number.isFinite(keys) && keys > 2 && <img src={getS3Image('key')} style={{ width: '25px', }} /> }
          </div>
          {!!archiveCards.length && (
            ...
            {handCards.map((card, i) => (
              <div
                style={{
                  ...
                  paddingLeft: handCards.length > 8 ? i === handCards.length - 1 ? '25px' : null : null,
                }}
            ...
            {archiveCards.map((card, i) => (
              <div
                style={{
                  ...
                  borderLeft: i === archiveCards.length - 1 ? '2px dashed #ccc' : null,
                }}
                key={`archives-${card}-${i}`}
              >
                <Card
