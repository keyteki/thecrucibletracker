...
class Games extends Component {
    ...
    render() {
        const { user, isMobile, games } = this.props;
        const { sortBy, sortDirection, showAllGames } = this.state;
        ...
        let deckNames = [];
        games.forEach((game) => {
            if (user === game.winner && !deckNames.includes(game.winner_deck_name))
                deckNames.push(game.winner_deck_name);
            if (user === game.loser && !deckNames.includes(game.loser_deck_name))
                deckNames.push(game.loser_deck_name);
        });

        deckNames = deckNames.sort((a, b) => {
            const deckAGames = games.filter(
                (g) =>
                    (g.winner === user && g.winner_deck_name === a) ||
                    (g.loser === user && g.loser_deck_name === a)
            );
            const deckBGames = games.filter(
                (g) =>
                    (g.winner === user && g.winner_deck_name === b) ||
                    (g.loser === user && g.loser_deck_name === b)
            );
            ...
            if (sortBy === 'Last Played') {
                ...
                deckAGames.forEach(
                    (g) => (deckAMaxDate = Math.max(deckAMaxDate, new Date(g.date).getTime()))
                );
                deckBGames.forEach(
                    (g) => (deckBMaxDate = Math.max(deckBMaxDate, new Date(g.date).getTime()))
                );

                return sortDirection === 'Descending'
                    ? deckBMaxDate - deckAMaxDate
                    : deckAMaxDate - deckBMaxDate;
            }

            if (sortBy === 'Games Played') {
                return sortDirection === 'Descending'
                    ? deckBGames.length - deckAGames.length
                    : deckAGames.length - deckBGames.length;
            }
        });
        ...
        let wins = 0;
        let losses = 0;
        const deckRows = deckNames
            .map((name) => {
                const deckGames = games.filter(
                    (g) =>
                        (g.winner === user && g.winner_deck_name === name) ||
                        (g.loser === user && g.loser_deck_name === name)
                );
                ...
                const deckID =
                    deckGames[0].winner === user
                        ? deckGames[0].winner_deck_id
                        : deckGames[0].loser_deck_id;
            ...
            <div style={{ display: 'flex' }}>
                ...
                >
                    ...
                    <FormControl style={{ marginRight: '20px', marginTop: '20px' }}>
                        ...
                        >
                            {['Games Played', 'Last Played'].map((key) => (
