  ...
  render() {
    ...
    const getCardData = (id) => {
      ...
      const output = {
        ...
        ...mm,
        ...dt,
        expansion: _.flatten([ dt.expansion, mm.expansion, wc.expansion, aoa.expansion, cota.expansion ]).filter((v) => v)
      };
      return output;
    };
    ...
    for (let i = 0; i < cards.length; i += 1) {
      cardEls.push(
        <div
          key={`${i}cards`}
          style={{
            display: 'flex', margin: '10px 0', justifyContent: 'space-between', width: '460px', overflow: 'hidden'
          }}
        >
          {[ i ].map((index) => {
            ...
            const firstPlayerEl = firstPlayer === 'anonymous' ? (
              <div style={{ fontSize: '16px', }}>
                {this.state.cardPlayData[card].topPlayers[0].count.toLocaleString()}
                {` for ${this.state.cardPlayData[card].topPlayers[0].player}`}
              </div>
            ) : (
              <div style={{ fontSize: '16px', }}>
                <a style={{ textDecoration: 'none' }} href={`/users/${firstPlayer}`}>{this.state.cardPlayData[card].topPlayers[0].player}</a>
                {' at '}
                {this.state.cardPlayData[card].topPlayers[0].count.toLocaleString()}
                {' plays'}
              </div>
            ...
            return (
              <div key={card} style={{ display: 'flex', padding: '10px', }}>
                <img style={{ width: '220px', height: '308px' }} src={getS3Image(filename, 'card')} />
                <div style={{ marginLeft: '20px' }}>
                  <div style={{ fontSize: '22px', marginTop: '10px' }}>
                    {getCardData(card).name}
                  </div>
                  <div style={{ fontSize: '12px', fontWeight: 'bold', marginTop: '20px', }}>
                    MOST PLAYS BY
                  </div>
                  {firstPlayerEl}
                  <div style={{ fontSize: '12px', fontWeight: 'bold', marginTop: '20px', }}>
                  ...
                  >
                    {this.state.cardPlayData[card].topPlayers.slice(1, 9).map(({ player, count }, j) => {
                      if (player === 'anonymous') {
                        return (
                          <div
                            style={{
                      ...
                      return (
                        ...
                        >
                          ...
                          <a style={{ textDecoration: 'none' }} target="_blank" href={`/users/${player}`}>{player}</a>
                        </div>
                      );
                    })}
                  </div>
